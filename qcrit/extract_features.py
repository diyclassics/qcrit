...
def _extract_features(corpus_dir, file_extension_to_parse_function, excluded_paths, features, output_file):
    ...
    if output_file is not None:
        ...
        # Temp csv option
        import csv
        with open(output_file.replace('.pickle', '.csv'), 'w', newline='') as file:
            writer = csv.writer(file)
            feats = list(list(text_to_features.values())[0].keys())
            row_1 = ['file']+feats
            writer.writerow(row_1)
            for item in text_to_features.items():
                row_feat = []
                for feat in feats:
                    row_feat.append(item[1][feat])
                writer.writerow([item[0]]+row_feat)
